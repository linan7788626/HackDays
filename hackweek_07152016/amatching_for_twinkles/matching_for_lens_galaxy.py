...
#----------------------------------------------------------------------------
    ...
    twinklesid = agn_twinkles_id[ind]
    ...
    print "Lens: [xc1, xc2, mag_tot, Reff_a, Reff_b, Theta, ndex, zl, lid]"
    print limg_cat
#----------------------------------------------------------------------------
    agns_cat = [agn_id[ind], agn_ra[ind], agn_dec[ind], agn_mag_norm[ind], agn_redshift[ind], agn_twinkles_id[ind], agn_image_num[ind], agn_lens_gal_id[ind]]

    print "AGN: [id, ra, dec, mag_norm, redshift, twinklesid, image_num, host_id]"
    print agns_cat
#----------------------------------------------------------------------------
    host_id, host_ra, host_dec, host_mag_norm, host_redshift, host_major_axis, host_minor_axis = np.loadtxt("./twinkles_DESC_SLAC/sprinkled_lens_galaxies_230.txt", comments='#', delimiter=',', converters=None, skiprows=1, usecols=None, unpack=True, ndmin=0)
    host_id = host_id.astype('int')

    idx = (host_id == agn_lens_gal_id[ind])

    host_cat = [host_id[idx][0], host_ra[idx][0], host_dec[idx][0], host_mag_norm[idx][0], host_redshift[idx][0], host_major_axis[idx][0], host_minor_axis[idx][0]]

    print "Host: [id, ra, dec, mag_norm, redshift, major_axis, minor_axis]"
    print host_cat
