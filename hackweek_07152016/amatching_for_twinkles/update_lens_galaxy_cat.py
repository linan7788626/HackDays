...
#
    ...
    print "# AGN_ID, AGN_RA, AGN_DEC, AGN_ABMAG, AGN_SED, AGN_RedShift, AGN_CatSim_ID, AGN_Twinkles_ID, AGN_IMG_NUM, AGN_SRC_y1, AGN_SRC_y2, AGN_IMG_MAG, LENS_IMG_SED"
    for i in xrange(len(agnId)):
        print agnId[i], agnRa[i], agnDec[i], agnMag[i], agnSed[i], agnRedshift[i], agnCatsim_id[i], agnTwinkles_id[i], agnImg_num[i], agnYs1[i], agnYs2[i], agnImgMag[i], LensImgSED[i]
