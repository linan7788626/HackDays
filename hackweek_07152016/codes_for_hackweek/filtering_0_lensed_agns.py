...
#----------------------------------------------------------------------------
    agn_id_2, agn_lensgal_id = np.loadtxt("./twinkles_DESC_SLAC/sprinkled_agn_230.txt", dtype="str", comments='#', delimiter=',', converters=None, skiprows=1, usecols=(0,7), unpack=True, ndmin=0)

    agn_lensgal_id = agn_lensgal_id.astype('int')
#----------------------------------------------------------------------------
...
#
    ...
    lsed = hdulist[1].data['lens_sed'][twinklesid]
    ...
    LensImgSED = lsed[idx]
    agnLensGalId = agn_lensgal_id[idx]
#----------------------------------------------------------------------------
    ...
    lgal_ra = lgal_ra.astype('double')
    lgal_dec = lgal_dec.astype('double')
#----------------------------------------------------------------------------
    print "# AGN_ID, AGN_RA, AGN_DEC, AGN_ABMAG, AGN_SED, AGN_RedShift, AGN_CatSim_ID, AGN_Twinkles_ID, AGN_IMG_NUM, AGN_SRC_y1, AGN_SRC_y2, AGN_IMG_MAG, LENS_IMG_SED, AGN_LensGal_ID"
    for i in xrange(len(agnId)):


        print agnId[i], agnRa[i], agnDec[i], agnMag[i], agnSed[i], agnRedshift[i], agnCatsim_id[i], agnTwinkles_id[i], agnImg_num[i], agnYs1[i], agnYs2[i], agnImgMag[i], LensImgSED[i], agnLensGalId[i]
