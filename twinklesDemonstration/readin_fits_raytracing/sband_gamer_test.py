...
def nie_phi(x1, x2, re0, rcore, qe):
    ...
    al2 = res0 * np.arctanh(x2 * np.sqrt(1 - qe * qe) /
                            (hfunc(x1, x2) + rcore * qe * qe))

    res1 = x1 * al1 + x2 * al2
    res2 = re0 * rcore * \
        np.log(np.sqrt((hfunc(x1, x2) + rcore)**2.0 + (1 - qe * qe) * x1 * x1))
    res = res1 - res2
    return res
...
def nie_alphas(x1, x2, re0, rcore, qe):
    ...
    al2 = np.arctanh(x2 * np.sqrt(1 - qe * qe) /
                     (hfunc(x1, x2) + rcore * qe * qe))
...
def nie_mu(x1, x2, re0, rcore, qe):
    res = 1.0 / (1.0 - re0 / hfunc(x1, x2) - re0 * re0 * rcore /
                 (hfunc(x1, x2) * ((hfunc(x1, x2) + rcore)**2 + (1 - qe * qe) * x1 * x1)))
    return res
...
def new_nie_all(xi1, xi2, lpar):
    ...
    a2 = b / np.sqrt(1 - q * q) * np.arctanh(x2 *
                                             np.sqrt(1 - q * q) / (wx + q * q * s))

    hx = np.sqrt((wx + s)**2.0 + (1 - q * q) * x1 * x1)
    phi = x1 * a1 + x2 * a2 - b * s * \
        np.log(hx) + b * q * s * np.log((1 + q) * s)
...
def find_critical_curve(mu):
    ...
    cdtn = np.sign(mu) * (np.sign(mu[rows - 1,
                                     cols]) + np.sign(mu[rows,
                                                         cols - 1]) + np.sign(mu[(rows + 1) % len(rows),
                                                                                 cols]) + np.sign(mu[rows,
                                                                                                     (cols + 1) % len(cols)]))
...
def main():
    ...
    g_snR = pyfits.getdata(
        "/Users/uranus/Desktop/gamer1/testims/HSTsource_0.003.highres.fits")
    g_snG = pyfits.getdata(
        "/Users/uranus/Desktop/gamer1/testims/HSTsource_0.003.highres.fits")
    g_snB = pyfits.getdata(
        "/Users/uranus/Desktop/gamer1/testims/HSTsource_0.003.highres.fits")

    #g_snR = pyfits.getdata(
        #"/Users/uranus/Desktop/gamer1/testims/HSTsource_0.003.lowres.fits")
    #g_snG = pyfits.getdata(
        #"/Users/uranus/Desktop/gamer1/testims/HSTsource_0.003.lowres.fits")
    #g_snB = pyfits.getdata(
        #"/Users/uranus/Desktop/gamer1/testims/HSTsource_0.003.lowres.fits")
    ...
    g_snR = np.array(g_snR, dtype="<d") * 102
    g_snG = np.array(g_snG, dtype="<d") * 204
    g_snB = np.array(g_snB, dtype="<d") * 255
