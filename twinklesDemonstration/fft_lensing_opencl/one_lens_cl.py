...
import pyopencl_convolution as pc
...
def fft_lensing_signals(kappac,green_in,dsx):
    ...
    convolver = pc.CachedQueueConvolver()
    batched_data = np.array([[kappac]])
    #print np.shape(batched_data)
    kernel = np.array([green_in])
    #print np.shape(kernel)
    phi = convolver.convolution(batched_data,kernel,'same')[0,0,:,:]*(dsx*dsx)
    #print np.shape(phi)
    ...
    #return phi,alpha1,alpha2,td,mu,kappas
...
def main():
    ...
    kappa=congrid.congrid(kappa0,[nnn,nnn])
    boxsize = 4.0
    ...
    phi,phi1,phi2,td,mu,kappas = fft_lensing_signals(sdens_pad,green_in,dsx)
    mu = mu.copy(order='C')
    ##----------------------------------------------------
    #green_in = green_iso(nnn,dsx)
    #phi,phi1,phi2,td,mu,kappas = fft_lensing_signals(kappa,green_in,dsx)
    ##----------------------------------------------------
