...
def calculate_lc_of_imgs():
    ...
    idx_nn = np.isnan(mags)
    mags_nn = mags[~idx_nn]
    time_nn = time_days[~idx_nn]-49570

    for i in xrange(len(time_nn)):
        pl.figure()
        pl.ylim(29,25)
        pl.xlabel("Days")
        pl.ylabel("Mags")
        pl.plot(time_nn,mags_nn,'k-o')
        pl.scatter(time_nn[i], mags_nn[i], s=400, facecolors='none', edgecolors='r',lw = 4)
        filename = "/Users/uranus/slider_pictures_shower/pngs"+'{:0>10}'.format(str(i))+"_sn.png"
        pl.savefig(filename)
