...
def lambda_e_prol(e):
    ...
    # else:
        # res = Intergral_ud(1.0,t_min, \
                       # lambda t: (np.sqrt((t*t*f2*f2+f2_p*f2_p) \
                                          # /(f2*(t*t-1)))/t)) \
        # / Intergral_ud(1.0,t_min, \
                       # lambda t: (((WX_prol_qt(t, f2)-WZ_prol_qt(t, f2))*t*t \
                                   # +WZ_prol_qt(t, f2))/np.sqrt(t*t-1)))
...
def e2le(e_in):
    ...
    e_tmp,lef_tmp = np.loadtxt("./ell_lef.txt",comments='#',usecols=(0,1),unpack=True)
...
if __name__ == '__main__':
    et = np.linspace(1e-6, 0.9, 100)


    # lto = np.array(map(lambda_e_obl, et))**2.0 - 1.0
    # ltp = np.array(map(lambda_e_prol, et))**2.0 - 1.0
    # lta = np.array(map(lambda_e_tot, et))
    # for i in xrange(100):
        # print et[i], lta[i]

    # et, lta = np.loadtxt("./ell_lef.txt", usecols=(0,1),unpack=True)
