...
class synthetic_image:
    ...
    def __init__(self,
            ...
            redshift=0.05,
            psf_fwhm_arcsec=1.0, pixelsize_arcsec=0.03,
            r_petro_kpc=None, save_fits=True,
            seed=None,
...
#============ SET ORIGINAL IMAGE ======================#
	...
	print np.sum(image)
	    ...
	    print self.band_name,bg_zpt[self.band]
            if len(bg_zpt[self.band]) > 0:
                image = image / ( 10.0**(-0.4*(bg_zpt[self.band][0]- 23.9 )) )
		print ( 10.0**(-0.4*(bg_zpt[self.band][0]- 23.9 )) )
        else:
...
#        print -2.5*np.log10( np.sum(image) )
        ...
        primhdu.header.update(ABABSZP=22.5,comment='For Final Image')  #THIS SHOULD BE CORRECT FOR NANOMAGGIE IMAGES ONLY
##       primhdu.header.update('ORIGZP',theobj.ab_abs_zeropoint,'For Original Image')
        primhdu.header.update(PIXSCALE=theobj.pixel_in_arcsec,comment='For Final Image, arcsec')
        primhdu.header.update(PIXORIG=theobj.camera_pixel_in_arcsec,comment='For Original Image, arcsec')
        primhdu.header.update(PIXKPC=theobj.pixel_in_kpc, comment='KPC')
        primhdu.header.update(ORIGKPC=self.sunrise_image.pixel_in_kpc,comment='For Original Image, KPC')
        ...
        primhdu.header.update(LUMDIST=self.cosmology.lum_dist, comment='MPC')
        primhdu.header.update(ANGDIST=self.cosmology.ang_diam_dist, comment='MPC')
        primhdu.header.update(PSCALE=self.cosmology.kpc_per_arcsec,comment='KPC')
        ...
        primhdu.header.update(PSFFWHM=self.telescope.psf_fwhm_arcsec,comment='arcsec')
        primhdu.header.update(TPIX=self.telescope.pixelsize_arcsec,comment='arcsec')
