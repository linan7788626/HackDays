...
void refine_critical(double * xi1,double * xi2,int nx1,int nx2,double * lpar,int npars,double * lpars, int nsubs,double * critical,int clen, int nfiner, double * yi1,double *yi2) {
    ...
    double dsf = dsx/nfiner/4;

    double * x1 = (double *)malloc(clen*sizeof(double));
    double * x2 = (double *)malloc(clen*sizeof(double));
	...
	for (i = 0; i < nx1; ++i) for (j = 0; j < nx2; ++j){
		...
		if (critical[index]>0) {
			...
			for (m = 0; m < nfiner; ++m) for (n = 0; n < nfiner; ++n){
        	    xt1[k*nfiner*nfiner+m*nfiner+n] = x1[k]+(dsf*(1-nfiner)*0.5)+dsf*m;
        	    xt2[k*nfiner*nfiner+m*nfiner+n] = x2[k]+(dsf*(1-nfiner)*0.5)+dsf*n;
			}
			k = k+1;
		}
	}
...
//------------------------------------------------------------------------
	...
	lanczos_diff_2_tag(al1,al2,a11,a12,a21,a22,dsx,nx1,0);
